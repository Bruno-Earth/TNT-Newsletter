...
description: Manually run the T&T VN, US, and global macro collectors, merge and validate their source inboxes, and save the results in the matching Monday-to-Sunday Google Drive folder. Use only when explicitly invoked for a current or demo collection run; never schedule it or continue into analysis or newsletter creation.
---
...
2. Inspect the destination's weekly folders for the latest successfully saved
   run. Verify its run-prefixed `run.json`, `validation.json`, and
   `save_receipt.json`; do not infer success from a weekly folder name alone.
...
1. Confirm the configured Drive destination and required write tools. Sharing
   status does not block collection; do not inspect or change permissions.
...
6. Use `run.json.drive_week.name` for the Monday-to-Sunday folder. Create that
   exact child under T&T Newsletter only when missing; otherwise reuse the one
   existing match. Save the run's uniquely named artifacts and readable Google
   Doc there, verify every write by metadata readback, and preserve prior runs.
7. Return the verified Google Docs inbox link, weekly-folder link, interval,
   retained counts, duplicates removed, coverage gaps, and save failures.
8. Stop.

If Drive access, collection, validation, or saving is blocked, preserve
